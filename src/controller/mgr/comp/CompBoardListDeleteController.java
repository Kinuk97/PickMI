package controller.mgr.comp;

import java.io.IOException;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import serivce.face.FileService;
import serivce.face.MgrService;
import serivce.impl.FileServiceImpl;
import serivce.impl.MgrServiceImpl;

/**
 * Servlet implementation class CompBoardListDeleteController
 */
@WebServlet("/compboard/listdelete/")
public class CompBoardListDeleteController extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private FileService fileService = FileServiceImpl.getInstance();

	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		
		MgrService mgrService = MgrServiceImpl.getInstance();
		
//		mgrService.listdeleteComp(req);
		
		resp.sendRedirect("mgr/complist");
		
	
		
	}
	
	
}
